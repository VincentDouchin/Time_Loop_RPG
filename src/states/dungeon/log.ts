...
export const logBundle = (split: boolean, entityInstance: LDTKEntityInstance, pos: Position) => (parent: Entity) => {
	if (split) {
		return parent.spawn(new Sprite(new PixelTexture(assets.staticItems.logSplit)), pos, new Log(), new YSorted(), entityInstance)
			.withChildren((log) => {
		...
		return parent.spawn(new Sprite(new PixelTexture(assets.staticItems.log)), pos, new YSorted(), ...entityInstance.body(), entityInstance, new Log())
	}
}